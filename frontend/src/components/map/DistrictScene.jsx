...
function LinkLine({ link, a, b, onSelect, inRoute, isValidNext, isSuggested, dimmed, affectedColor }) {
  ...
  const color = linkStrokeColor({ broken, inRoute, isValidNext, isSuggested, link })
  const mid = points[Math.floor(points.length / 2)]
  const lineWidth = linkStrokeWidth({ broken, inRoute, isValidNext, isSuggested, status: link.status, isGround })
  const opacity = linkStrokeOpacity({ dimmed, broken, inRoute, isValidNext, isSuggested, isGround })
  const selectLink = (e) => {
    e.stopPropagation()
